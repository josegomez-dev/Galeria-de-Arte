...
public class Pintor {
	...
	public ArrayList<Pintor> getMaestros() throws SQLException, Exception{
		if(maestros.size() == 0){
			setMaestros((new MultiPintor()).listarMaestrosXPintor(this.getCedula()));
		}
		return maestros;
	}
	...
	public ArrayList<Pintura> getObras() throws SQLException, Exception{
		if(obras.size() == 0){
			setObras((new MultiPintura()).consultarXPintor(this.getCedula()));
		}
		return obras;
	}

	private void setObras(ArrayList<Pintura> pmaestros){
		obras = pmaestros;
	}
